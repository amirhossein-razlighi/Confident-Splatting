...
def tiles_info(panoid, zoom=5):
    ...
    tiles = [
        (x, y, "%s_%dx%d.jpg" % (panoid, x, y), image_url.format(panoid, zoom, x, y))
        for x, y in coord
    ]
    return tiles
        ...
        if x * tile_width < img_w and y * tile_height < img_h:  # tile is valid
            while True:
                try:
                    ...
                    tile_img.verify()  # Verify if the image is not corrupted
                    tile_img = Image.open(
                    ...
                    print(f"Warning: Failed to load tile {x}, {y}. Using a blank tile.")
                    blank_tile = Image.new(
                        "RGB", (tile_width, tile_height), (0, 0, 0)
                    )  # Black tile
                    valid_tiles.append(blank_tile)
