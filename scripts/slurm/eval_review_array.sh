#!/usr/bin/env bash
#SBATCH --job-name=cs_review_eval
...
CKPT="$(find "${RESULT_DIR}/ckpts" -maxdepth 1 -name 'ckpt_*_rank0.pt' | sort -V | tail -n 1)"
if [[ -z "${CKPT}" ]]; then
  echo "No checkpoint found in ${RESULT_DIR}/ckpts" >&2
  exit 2
fi

COMMON_EVAL_ARGS="${COMMON_EVAL_ARGS:---disable-viewer --skip-render-traj --eval-random-trials 3}"
read -r -a COMMON_ARGS <<< "${COMMON_EVAL_ARGS}"
...
echo "Common eval args: ${COMMON_EVAL_ARGS}"
